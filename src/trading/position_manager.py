...
class PositionManager:
    ...
    def can_open_position(self, category: str) -> bool:
        ...
        category_positions = len([
            ...
            if p.category == category and p.status == "ACTIVE"
        ])
        ...
        if category == "MEME":
            ...
            if category_positions >= MAX_AI_POSITIONS:
                self.logger.info(f"At max AI positions: {category_positions}/{MAX_AI_POSITIONS}")
                return False
    ...
    def set_stop_loss(self, entry_price: float) -> float:
        """Set stop loss level"""
        return entry_price * 0.9  # 10% stop loss
    ...
    def get_position_summary(self) -> dict:
        ...
        return {
            ...
            "by_category": {
                "AI": len([p for p in active if p.category == "AI"]),
                "MEME": len([p for p in active if p.category == "MEME"]),
                "HYBRID": len([p for p in active if p.category == "HYBRID"])
            }
        }
